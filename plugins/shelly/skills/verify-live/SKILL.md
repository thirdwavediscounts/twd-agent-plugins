...
1. **Signed in as the verify account.** Drives run as
   ...
   ```
   # ESM resolves @supabase/supabase-js by walking up from the script's dir, so
   # run it FROM the app tree (NODE_PATH does not work for ESM):
   cp <skill>/scripts/*.mjs <monorepo>/apps/product-research/.verify-boot/
   node <monorepo>/apps/product-research/.verify-boot/bootstrap-session.mjs
   ```
   ...
   Google, no cookie built by hand — `bootstrap-session.mjs` captures the exact
   string the app's own client persists and `synth-session-cookie.mjs` lays it
   into the `twd-auth.*` cookie the app reads. `node synth-session-cookie.mjs`
   runs the format self-test (no creds needed). The `.verify-boot/` scratch dir
   (gitignored under the app) also holds the Playwright driver and fixtures;
   delete it in cleanup. Every write the drive makes carries
...
session — `chromium.launch()` → `newContext({ storageState, recordVideo: { dir,
size } })` → `addInitScript` to set the staging switch before the app's JS runs
...
gives the `.webm` — save it as evidence (below). The "import complete" toast is
easy to miss on timing; trust the DB row, not the banner. Prefer stable handles
(`getByRole`, `input[type=file]`) over coordinates.
