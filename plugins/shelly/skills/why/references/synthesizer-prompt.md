...
2. Every Direct/Supported claim must have a citation (PR #, `DEV-nnn` ticket, Bugsink issue ID, audit_logs row, journal timestamp, commit hash, or file:line).
...
2. **Reconcile overlapping findings.** Multiple investigators may have cited the same PR, ticket, or issue. Merge into a single, authoritative reference.
...
5. **Verify citations by spot-checking.** You can read the codebase and call reachable tools (Linear, Supabase MCP, ssh) to verify citations; do not write files, commit, or modify external state. If you're uncertain a cited item exists or says what's claimed, check it. Don't propagate errors.
...
- **[Direct]** {Claim}. Source: [PR #123](url) / `DEV-nnn` / Bugsink issue ID / file:line. {Brief quote or paraphrase.}
...
**Explicit gaps.** Things the user asked that the evidence didn't answer. Sources searched that came up empty. Sources that weren't reachable at all, such as an unresponsive ssh host.

Be specific. "We searched Linear for [query1], [query2], [query3] and found no issue discussing the rate-limit threshold" is useful. "We don't know why" is not. Include:

- Specific questions that went unanswered
- Searches that returned nothing
- Sources that were unreachable (and why)
...
- **Linear**: {ticket IDs and keyword searches}. Or "Not searched. `mcp__linear__*` tools were not reachable this session."
- **Bugsink**: {project, issue IDs, and queries run via `bugsink-manage shell`}. Or "Not searched. `ssh ken-ai-agents` did not respond this session."
- **Supabase audit logs**: {tables and time windows queried against `audit_logs`}. Or "Not searched. `supabase-production` MCP was not reachable this session."
- **VPS journal**: {units and time windows checked via `journalctl`}. Or "Not searched. `ssh ken-ai-agents` did not respond this session."

### Confidence Summary

One or two sentences summarizing your overall confidence. E.g.:

> "The core rationale (A) is well-supported by direct PR and Linear ticket evidence. The specific threshold value (100) is inferred from the surrounding Bugsink issue context but not explicitly documented. Whether this was driven by a specific customer's data could not be answered; Supabase audit logs showed no writes to the affected table in the relevant window."
