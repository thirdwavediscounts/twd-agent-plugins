...
while true; do
  ...
  threads=$(gh api graphql -f query='query($n:Int!){repository(owner:"thirdwavediscounts",name:"twd-apps-monorepo"){pullRequest(number:$n){reviewThreads(first:100){nodes{isResolved}}}}}' -F n="$pr" --jq '[.data.repository.pullRequest.reviewThreads.nodes[] | select(.isResolved|not)] | length' 2>/dev/null || echo "?")
