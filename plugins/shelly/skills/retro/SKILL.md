...
- **Standards → review agent** — a coding rule sitting in always-loaded steering that the review agents (`plugins/shelly/agents/code-reviewer.md`, `qa-gate.md`, `security-auditor.md`) or `packages/guidelines/EXAMPLES.md` should own instead. *Fires when* a defect slipped past review, or an implementer-facing rule is really a review concern.
