...
- **Prose → structural enforcement** — a rule enforced more reliably by a hook, a policy test (`apps/*/tests/*Policy.test.ts`), a lint, or `ci-local` than by words. Text is the weakest enforcement. *Fires when* the session broke a rule that prose was supposed to hold.
